...
process_folder="${run_folder}/run_files.preproc"
process_template="template.preproc"
...
cat ${subject_list} | while read line; do
	sed -e "s|SUB|${line}|g" -e "s|SES|${ses}|g" -e "s|BASE|${project_dir}|g" ${run_folder}/${process_template} > ${process_folder}/run${k}
...
sed -e "s|GROUP|${group}|g" -e "s|EMAIL|${email}|g" -i ${run_folder}/resources_preproc.sh 
