...
tmp_dir="/tmp" # working dir
bids_dir="/projects/standard/lewi1538/shared/projects/neg_urg/derivatives/nordic"
output_bucket="s3://abflab"
run_folder=`pwd`
...
process_folder="${run_folder}/run_files.fmriprep"
...
cat ${subject_list} | while read line; do
	sed -e "s|SUBID|${line}|g" -e "s|SESID|${ses}|g" -e "s|INPUT|${bids_dir}|g" -e "s|OUTPUT|${output_bucket}|g" -e "s|DATADIR|${tmp_dir}|g" -e "s|RUNDIR|${run_folder}|g" ${run_folder}/${process_template} > ${process_folder}/run${k}
